...
void setup() {
  ...
  if (testWiFi()) {  // Try connecting to WiFi
    ...
    const char* ssidap = "ESP8266Wifi";  // Define SSID for AP mode
    ...
    Serial.print("Please connect to ESP8266Wifi and access to http://");
...
void createWebServer(int webtype) {
  server.on("/", [webtype]() {
    ...
    if (webtype == 0) {  // Client mode
      content += "<h1>WiFi Mode</h1><h2>Welcome to Your WiFi</h2>";
      ...
      content += "<h1>Access Point Mode</h1><h2>NodeMCU WiFi Configuration</h2>";
      content += "<p><b>Current Configuration:</b></p>";
      content += "<table border='1'><tr><td>SSID</td><td>" + ssid + "</td></tr>";
      ...
      content += "</td></tr>";
      content += "<tr><td>Temperature</td><td><span id='temp'>" + String(temp, 1) + " C</span></td></tr>";  // Real-time temperature
      content += "<tr><td>Humidity</td><td><span id='hum'>" + String(hum, 1) + " %</span></td></tr>";       // Real-time humidity
  ...
  server.on("/setting", []() {
    ...
    content += "body { font-family: Arial; text-align: center; }";
    content += "</style></head><body>";
    content += "Configuration saved!<br>";
